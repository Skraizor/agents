...
targets or CI. The only "application" is four idempotent installer scripts that symlink the
...
- Installers create **symlinks** back into this repo, so editing a source file under
  `agents/`, `codex-agents/`, `cursor-agents/`, or `opencode-agents/` takes effect immediately
  for new host sessions — no reinstall needed. Re-run the relevant installer only after
  **adding, renaming, or deleting** an agent file.
